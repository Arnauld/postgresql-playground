...
public class FF3Cipher {
  ...
  private static Map<String, BigInteger> pows = new ConcurrentHashMap<>();

  static BigInteger pow(int base, int exponent) {
    String key = base + ":" + exponent;
    BigInteger bp = pows.get(key);
    if (bp == null) {
  ...
  protected static BigInteger decode_int(String str, String alphabet) {
    ...
    for (char each : str.toCharArray()) {
      ...
      num = num.add(pow(base, exponent).multiply(BigInteger.valueOf(alphabet.indexOf(each))));
      idx += 1;
    }
    return num;
  }
  ...
  private static final Logger logger = LogManager.getLogger(com.privacylogistics.FF3Cipher.class.getName());
